...
typedef enum {
    SNOW_MODEL = 0,
    ICE_MODEL = 1,
    CS_PENETRATION_MODEL = 2
}

sealand_model_t;

const float DENSITY_WATER = 997.0;
...
int gvcart_initialise_(int * nmodels,
                       int * nhierarchical) {
    * nmodels = 3; // Snow thickness, Ice thickness, and AltiKa penetration factor
    * nhierarchical = 1; // AltiKa Observations
    ...
    for (int i = 0; i < nobs; i++) {
        ...
        switch (obs[i].type) {
            case CS2_FREEBOARD_OBS:
                ...
                // We will start with these two parameters for now
                mi[0] = SNOW_MODEL;
                mi[1] = ICE_MODEL;
                mi[2] = CS_PENETRATION_MODEL;
...
int gvcart_compute_prediction_(int * nmodels,
    ...
    switch (obs[ * observation].type) {
        case CS2_FREEBOARD_OBS: // AK Freeboard Observations, we have  two models loaded, snow->value[0], ice->value[1], penetration->value[2]
