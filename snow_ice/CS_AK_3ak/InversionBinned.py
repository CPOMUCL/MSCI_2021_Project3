...
def main(fb_path1="data/CPOM/freeboard_daily_processed/CS2_CPOM/dailyFB_50km_2013-2014_season.pkl",
    ...
    for i in range(shape[0]):
        ...
        for j in range(shape[1]):
            ...
            for date in list(fb1.keys())[:30]:
                if not np.isnan(fb1[date][i][j]):
                    cs2_observations.append([grid_x[i][j], grid_y[i][j], 0, fb1[date][i][j], 0.1])                    

                if not np.isnan(fb2[date][i][j]):
                    ak_observations.append([grid_x[i][j], grid_y[i][j], 1, fb2[date][i][j], 0.1])
    ...
    if render_map:
        ...
        plt.colorbar(label=r'Ice Thickenss (m)')
